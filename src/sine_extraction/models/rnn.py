"""Vanilla RNN model for sine-wave extraction."""
...
class RNNModel(BaseModel):
    """Recurrent network (vanilla RNN) for sequence-to-sequence prediction.
    ...
    Args:
        config: RNN architecture settings (hidden_size, num_layers).
        window_size: Number of samples per input/output window (unused at
            construction but kept for API consistency with MLPModel).
    """

    def __init__(self, config: RNNConfig, window_size: int) -> None:  # noqa: ARG002
        super().__init__()
        self.rnn = nn.RNN(
